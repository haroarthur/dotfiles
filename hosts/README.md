...
| `common.nix` | Shared out-of-store links (WezTerm included) + Claude session flags. Points `~/.claude/skills` at `~/.agents/skills`. **Seeds**, never links: the Herdr config, and Claude's and Pi's `settings.json` - each is rewritten by the program that owns it. |
| `mac.nix` | The Mac Home Manager user (wired into `darwinConfigurations.mac`). Imports `../pkgs/shell.nix` - the same shell the WSL host comes up in - and adds the font and fontconfig. Never `../pkgs/shell-wsl.nix`. |
